...
function renderOverview(data){
    const stats=`<div style="display:flex;gap:12px;align-items:center;justify-content:space-between;margin-bottom:12px">
        ...
        <div class="progress" style="height:12px;border-radius:12px;margin-top:6px"><i style="width:${data.total_expected?Math.round((data.submitted_count/data.total_expected)*100):0}%"></i></div></div>
    </div>`;
    ...
    tableWrap.innerHTML=`
        <form id="marksForm">
            <table class="table" role="table" aria-label="Submissions table">
                <thead>
                    <tr>
                        <th>Student Name</th><th>Reg No</th><th>Submission</th><th>Marks (out of 100)</th><th>Graded</th><th>Comment</th>
                    </tr>
                </thead>
                <tbody>${rows}</tbody>
            </table>
