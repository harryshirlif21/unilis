...
<div id="scheduleMeetingModal" class="modal hidden">
    <div class="modal-content bg-white rounded-2xl border border-f5e6b2 shadow-2xl" style="max-width: 580px; max-height: 92vh; overflow-y: auto;">
        ...
        <form action="../actions.php" method="POST">
            ...
            <div class="modal-body">
                <?php
                $meetingAppBaseUrl = rtrim(getMeetingAppBaseUrl(), '/');
                $hostPreviewBase = $meetingAppBaseUrl . '/lecturer/meeting_host.php?meeting_id=';
                $studentPreviewBase = $meetingAppBaseUrl . '/student/meeting_join.php?meeting_id=';
                ?>
                <div class="form-group">
                ...
                <div class="form-group">
                    ...
                    <input type="url" name="meeting_link" id="meetingLinkInput" placeholder="Leave blank to auto-generate a UNILIS meeting link">
                    <p class="text-xs text-gray-500 mt-2">If you leave this blank, UNILIS will generate a shareable meeting link automatically.</p>
                </div>
                <div class="form-group bg-gray-50 border border-gray-200 rounded-xl p-4">
                    <label class="block text-sm font-semibold text-gray-700 mb-3">Generated Link Preview</label>
                    <div class="space-y-3">
                        <div>
                            <label class="block text-xs uppercase tracking-wide text-gray-500 mb-1">Host link</label>
                            <input
                                type="text"
                                id="meetingHostPreview"
                                readonly
                                data-auto-base="<?= htmlspecialchars($hostPreviewBase) ?>"
                                value="<?= htmlspecialchars($hostPreviewBase . 'auto-generated') ?>"
                            >
                        </div>
                        <div>
                            <label class="block text-xs uppercase tracking-wide text-gray-500 mb-1">Student link</label>
                            <input
                                type="text"
                                id="meetingStudentPreview"
                                readonly
                                data-auto-base="<?= htmlspecialchars($studentPreviewBase) ?>"
                                value="<?= htmlspecialchars($studentPreviewBase . 'auto-generated') ?>"
                            >
                        </div>
                    </div>
                    <p class="text-xs text-gray-500 mt-3" id="meetingPreviewHint">The real meeting ID is assigned when you click Schedule Meeting.</p>
                </div>
                <div class="form-group">
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    const meetingHostPreview = document.getElementById('meetingHostPreview');
    const meetingStudentPreview = document.getElementById('meetingStudentPreview');
    const meetingPreviewHint = document.getElementById('meetingPreviewHint');

    function updateMeetingLinkPreview() {
        if (!meetingHostPreview || !meetingStudentPreview) {
            return;
        }

        const manualLink = (meetingLinkInput?.value || '').trim();
        const hostAutoBase = meetingHostPreview.dataset.autoBase || '';
        const studentAutoBase = meetingStudentPreview.dataset.autoBase || '';

        if (manualLink !== '') {
            meetingHostPreview.value = hostAutoBase + 'auto-generated';
            meetingStudentPreview.value = manualLink;
            if (meetingPreviewHint) {
                meetingPreviewHint.textContent = 'Students will open the link you entered. The host link remains auto-generated by UNILIS.';
            }
            return;
        }

        meetingHostPreview.value = hostAutoBase + 'auto-generated';
        meetingStudentPreview.value = studentAutoBase + 'auto-generated';
        if (meetingPreviewHint) {
            meetingPreviewHint.textContent = 'The real meeting ID is assigned when you click Schedule Meeting.';
        }
    }
    ...
    meetingLinkInput?.addEventListener('input', updateMeetingLinkPreview);
