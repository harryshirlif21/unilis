...
<header class="header">
    ...
    <button class="hamburger-menu" id="hamburgerMenu" type="button"><i class="fas fa-bars"></i></button>
</header>
...
<div class="recent-activity-section">
    ...
    <div class="recent-activity-section">
        <h3>Interactive Assignments / CATs</h3>
        <div class="table-container">
            <div style="margin-bottom:10px">
                <label><b>Filter by Unit:</b></label>
                <select id="ia-unit-filter" class="input" style="max-width:320px">
                ...
                <tbody>
                    ...
                    try {
                        ...
                        if ($int_assignments->num_rows === 0) {
                            echo "<tr><td colspan='4'>No interactive assignments or CATs for your class.</td></tr>";
                        } else {
                            while ($ia = $int_assignments->fetch_assoc()) {
                                ...
                                $action = $submitted
                                    ...
                                    : "<a href='take_assignment.php?id=" . $ia['id'] . "' class='action-link'>Answer MCQs</a>";

                                echo "<tr>
                                    <td>" . htmlspecialchars($ia['unit_name']) . "</td>
                                    <td>" . htmlspecialchars($ia['title']) . "</td>
                                    <td>" . date("d M Y, h:i A", strtotime($ia['due_date'])) . "</td>
                                    <td>$action</td>
                                </tr>";
                            }
                        }
                        ...
                        echo "<tr><td colspan='4'>Error loading interactive assignments.</td></tr>";
                    }
                    ?>
                </tbody>
            </table>
...
<script>
    ...
    (function(){
        ...
        if (hamburger) hamburger.addEventListener('click', function(e){ e.preventDefault(); openMenu(); });
    ...
    (function(){
        ...
        sel.addEventListener('change', function(){
            ...
            const tbody = this.closest('.table-container').querySelector('tbody');
            if (!unitId) return;
            ...
            fetch(`../actions.php?action=get_interactive_assignments_by_unit&unit_id=${unitId}`)
              ...
              .then(data=>{
                ...
                if (!items.length) { tbody.innerHTML = '<tr><td colspan="4">No interactive assignments for this unit.</td></tr>'; return; }
                tbody.innerHTML = '';
                items.forEach(a => {
                    const d = a.due_date ? new Date(a.due_date).toLocaleString() : '';
                    const tr = document.createElement('tr');
                    tr.innerHTML = `<td>-</td><td>${a.title}</td><td>${d}</td><td><a href="take_assignment.php?id=${a.id}" class="action-link">Answer MCQs</a></td>`;
