<?php
/**
 * Live Engagement Database Setup (Standalone)
 *
 * This is a standalone version of the setup script placed at the root
 * of the application to avoid 404 issues when the modules/ directory
 ...
 *
 * Access: https://unilis.jhubafrica.com/setup_live_engagement.php
 */
...
try {
    // Load the application database connection
    require_once __DIR__ . '/config/db.php';

    // Load the Live Engagement database installer
    define('LIVE_ENGAGEMENT_INSTALL', true);
    require_once __DIR__ . '/modules/live-engagement/database/install.php';

    $result = installLiveEngagementTables($conn);
    $messages = $result['messages'];

    if ($result['success']) {
        // Create le_guest_users table
    ...
    echo json_encode([
        ...
        'messages' => $messages,
        'next_step' => $result['success'] ? 'Delete setup_live_engagement.php after confirming setup.' : 'Review the error above and run this page again.',
