    ...
    <div class="grid gap-6 md:grid-cols-2">
      <section id="create" class="rounded-2xl bg-white p-7 shadow-sm ring-1 ring-slate-200"><h2 class="text-2xl font-bold">Create a meeting</h2>
        <form class="mt-6 space-y-4" method="post" action="actions.php"><input type="hidden" name="action" value="schedule_open_meeting">
          <label class="block text-sm font-medium">Your name<input class="mt-1 w-full rounded-lg border border-slate-300 p-3" name="host_name" required maxlength="120" value="<?= htmlspecialchars($_SESSION['user_name'] ?? '') ?>"></label>
        ...
        <p class="mt-4 text-sm text-slate-500">No account or unit selection is required. Keep the private host link shown after creation.</p>
      </section>
