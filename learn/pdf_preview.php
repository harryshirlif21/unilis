...
if (!preg_match('#^uploads/course_pdfs/[A-Za-z0-9._-]+\.pdf$#i', $file)) {
    error_log("learn pdf_preview: Invalid file pattern - $file");
    http_response_code(400);
    exit('Invalid PDF file.');
}

$absolute = realpath(__DIR__ . '/../' . $file);
$uploads = realpath(__DIR__ . '/../uploads/course_pdfs');

// Debug logging
error_log("learn pdf_preview: file=$file, absolute=" . ($absolute ?: 'false') . ", uploads=" . ($uploads ?: 'false'));

if ($absolute === false) {
    error_log("learn pdf_preview: File not found at path: " . __DIR__ . '/../' . $file);
    http_response_code(404);
    exit('PDF not found.');
}

if ($uploads === false) {
    error_log("learn pdf_preview: Uploads directory not found: " . __DIR__ . '/../uploads/course_pdfs');
    http_response_code(404);
...
if (strpos($normalizedAbsolute, $normalizedUploads . '/') !== 0) {
    error_log("learn pdf_preview: Path traversal detected - absolute=$normalizedAbsolute, uploads=$normalizedUploads");
    http_response_code(404);
    exit('PDF not found.');
}
...
$basePath = rtrim(str_replace('\\', '/', dirname(dirname($_SERVER['SCRIPT_NAME'] ?? '/learn/pdf_preview.php'))), '/');
