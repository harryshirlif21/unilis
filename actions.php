...
if ($action === 'schedule_open_meeting') {
    ...
    $host_name = trim($_POST['host_name'] ?? ($_SESSION['user_name'] ?? ''));
    $scheduled_time = trim($_POST['scheduled_time'] ?? '');
    $duration = max(15, min(480, (int)($_POST['duration'] ?? 60)));
    $session_role = (string)($_SESSION['user_role'] ?? 'guest');
    $host_user_id = isset($_SESSION['user_id']) ? (int)$_SESSION['user_id'] : null;
    $lecturer_id = $session_role === 'lecturer' ? $host_user_id : null;

    if ($title === '' || $host_name === '' || $scheduled_time === '' || strtotime($scheduled_time) === false || strtotime($scheduled_time) < time() - 60) {
        $_SESSION['open_meeting_error'] = 'Enter your name, a title, and a future meeting time.';
    ...
    foreach (['lecturer_id', 'host_user_id', 'host_role', 'host_name', 'host_token', 'guest_access', 'guest_listed', 'guest_token'] as $requiredColumn) {
        $column = $conn->query("SHOW COLUMNS FROM meetings LIKE '" . $conn->real_escape_string($requiredColumn) . "'");
    ...
    try {
        ...
        $stmt = $conn->prepare('INSERT INTO meetings (lecturer_id, unit_id, host_user_id, host_role, host_name, host_token, title, meeting_link, scheduled_time, duration, guest_access, guest_listed, guest_token) VALUES (?, NULL, ?, ?, ?, ?, ?, ?, ?, ?, 1, 1, ?)');
        $stmt->bind_param('iissssssis', $lecturer_id, $host_user_id, $session_role, $host_name, $hostToken, $title, $emptyLink, $scheduled_time, $duration, $guestToken);
        $stmt->execute();
        $meeting_id = (int)$conn->insert_id;
        $stmt->close();

        $guestLink = getMeetingGuestUrl($guestToken);
        $stmt = $conn->prepare('UPDATE meetings SET meeting_link = ? WHERE id = ?');
        $stmt->bind_param('si', $guestLink, $meeting_id);
        $stmt->execute();
        $stmt->close();

        $_SESSION['open_meeting_success'] = 'Meeting created. Share the invitation code or link with participants.';
        $_SESSION['open_meeting_host_link'] = getMeetingHostUrl($meeting_id) . '&host_token=' . rawurlencode($hostToken);
        $_SESSION['open_meeting_guest_link'] = $guestLink;
        $_SESSION['open_meeting_code'] = $guestToken;
    } catch (Throwable $e) {
